...
from gym_pybullet_drones.utils.control import RotToQuat, quatMultiply, quatInverse, quat2Dcm
...
class PX4Control(BaseControl):
    ...
    def z_vel_control(self):
        ...
        stop_int_D = (thrust_z_sp >= uMax
                      and vel_z_error >= 0.0) or (thrust_z_sp <= uMin
                                                  and vel_z_error <= 0.0)
        ...
        if not (stop_int_D):
            ...
            self.thr_int[2] = min(abs(self.thr_int[2]), self.maxThr) * np.sign(
                self.thr_int[2])
    ...
    def xy_vel_control(self):
        ...
        thrust_max_xy = math.sqrt(self.maxThr**2 - self.thrust_sp[2]**2)
    ...
    def attitude_control(self):
        ...
        self.rate_sp += quat2Dcm(quatInverse(self.quat))[:, 2] * self.yawFF
